...
int dfs(int cr, int cc) {
    ...
    if (dp[cr][cc] != -1) //이미 값(현재 정점에서 오른쪽 아래 끝까지 가는 경로의 수)이 존재한다면
        return dp[cr][cc]; // 해당 값 리턴

    dp[cr][cc] = 0; //탐색 시작을 위해 0으로 초기화
    for (int i = 0; i < 4; i++) { // 상하좌우 방향 탐색
        int nr = cr + dr[i]; // 상 하로 이동하며 row 좌표 리셋
        int nc = cc + dc[i]; // 좌 우로 이동하며 col 좌표 리셋
