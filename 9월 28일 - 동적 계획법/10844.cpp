...
int easyStairNum(int n) {
    vector<vector<int>> dp(n + 1, vector<int>(12, 0)); // 길이가 (n+1)일 때 마지막 수가 vector..일 경우의 계단 수
    int ans = 0; // 최종 값

    //1의 자리 쉬운 계단 수 먼저 초기화.
    for (int i = 2; i <= 10; i++) //i: 끝나는 수 (1 ~ 9)
        dp[1][i] = 1; // 길이가 1인 경우에는 범위 내 끝나는 수에 관계 없이 계단 수가 1이다. (왜? 한 자리 수는 모두 계단수)

    //자릿수마다 j로 끝나는 쉬운 계단 수의 개수 구하기
    for (int i = 2; i <= n; i++) { //i: 자릿수
        for (int j = 1; j <= 10; j++) { //j: 끝나는 수 (0 ~ 9)
            dp[i][j] = (dp[i - 1][j - 1] + dp[i - 1][j + 1]) % 1000000000;
            // dp[i]의 끝나는 수가 2면 (j = 2), dp[i - 1]의 끝나는 수는 1(j - 1)이나 3(j + 1)이었을 것. (역으로 생각!)
        }
    }

    for (int i = 1; i <= 10; i++) // i: 끝나는 수 (0 ~ 9)
        ans = (ans + dp[n][i]) % 1000000000;
        // 정답은 n자리까지 끝나는 수의 합을 1,000,000,000으로 나눈 나머지
