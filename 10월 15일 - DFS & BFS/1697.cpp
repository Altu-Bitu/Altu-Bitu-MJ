...
int bfs() {
    ...
    while(!q.empty()) {
        ...
        if (N == K) // 동생과 같은 위치에 도착했다면
            return time - 1; // 걸린 시간 반환 (visited[N]의 초기값이 1이었기 때문에 1 빼준다.)
        if(N > 0 && visited[N - 1] == 0) { // 만약 이전 칸으로 이동했다면 (N - 1)이 음수가 될 수 없기 때문에 양수여야 한다.
            q.push(N - 1); // 큐에  추가
            visited[N - 1] = visited[N] + 1; // 해당 위치 방문 시간 기록
        }
        if (N < 100000 && visited[N + 1] == 0) { // 만약 이후 칸으로 이동했다면 (N + 1이 최대값을 넘어갈 수 없기 때문에 제한 설정_
            q.push(N + 1); // 큐에 추가
            visited[N + 1] = visited[N] + 1; // 해당 위치 방문 시간 기록
        }
        if (N * 2 <= 100000 && visited[N * 2] == 0) { // 만약 순간이동을 했다면 (마찬가지로 위치 범위 제한)
            q.push(N * 2); // 큐에 추가
