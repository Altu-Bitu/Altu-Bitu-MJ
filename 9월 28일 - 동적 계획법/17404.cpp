 ...
 * 조건(1149번과 다른 점): N번째 집의 색은 (N - 1)번째, (N + 1)번째의 집의 색과 달라야 한다.
 * + 1번째 집은 2번째, N번째와 달라야 한다.
*/
#include <iostream>
...
int totalCost()
{
    for (int k = 0; k < 3; k++)
    {
        ...
        for (int j = 0; j < 3; j++) {
            if (j == k) continue;
            finalMin = min(finalMin, DP[N - 1][j]);
        }
    }
